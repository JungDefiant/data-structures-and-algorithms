...
function sayHello(request, response){
  // request is getting the information at /hello
  // response is returning back to the client
...
const oddValues = (arr) => {
  return arr.filter(val => { return (val % 2) !== 0; });
};
...
const filterStringsWithVowels = (arr) => {
  return arr.filter(val => {
    // val is an element from array
    // If val only has vowels, that means it must contain aeiou
    // val.match(/[aeiou]/g) creates an array of all vowels in the string
    // so if val has one or more vowels, then val.match(/[aeiou]/g) returns an array
    let matchArr = val.match(/[aeiou]/g);
