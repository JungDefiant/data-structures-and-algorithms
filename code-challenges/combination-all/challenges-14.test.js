...
const createServer = () => {
  ...
  app.delete('/things/1', (req, res) => res.sendStatus(405));
...
let biggerThanLuke = (arr) => {
  ...
  const lukeWeight = arr[0].mass;
  arr.forEach((char, ind) => {
    if(parseInt(char.mass) > lukeWeight) {
      if(characterString.length > 0) characterString += " - "; 
      characterString += char.name;
    }
  });
    ...
    for(let j = i + 1; j < arr.length; j++) {
      ...
      if(item1 && item2) {
        if(item1[property] > item2[property]) {
          let itemToSwitch = item2;
          console.log(item1[property] + " is greater than " + item2[property]);
          arr[j] = item1;
          arr[i] = itemToSwitch;
          j++;
        }
      } else console.log("Can't find item or property!");
    }
  }
  console.log(arr);
  return arr;
