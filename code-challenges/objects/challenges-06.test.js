...
const templatingWithMustache = () => {
  let charHTMLArray = [];
  characters.forEach(charObject => charHTMLArray.push(Mustache.render($.html('body'), charObject)));
  return charHTMLArray;
}
...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(charObj => houses.push(charObj['house']));
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  // arr is the array of characters
  let hasChildren = false;
  arr.forEach(charObj => { 
    if(charObj['name'] === character) hasChildren = (Object.values(charObj, 'children').length > 0); 
  });
  return hasChildren;
};
