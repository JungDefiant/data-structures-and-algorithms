...
const isCapitalized = (str) => {
  // Solution code here...
  const x = str.toString().match(/\b[A-Z][a-z]+/g)
  if (x !== null) {
    return x;
  } else {
    return [];
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  // const x = arr.toString().match(/([A-J][\w\s]+)[^\w\.]/g);
  const x = [];
  arr.forEach(str => {
    let newStr = str.match(/(^[A-J][\w\s]+$)/g);
    if (newStr !== null) {
      let strToAdd = "";
      for (var s in newStr) strToAdd = strToAdd + newStr[s];
      x.push(strToAdd);
    }
  })
  return x;
...
const matchMonth = (input) => {
  const correctInput = ['October', 'Oct', 'october', 'oct'];
  const inputMatch = input.toString().match(/([Oo]ct(ober)?)/g);
  correctInput.forEach((str) => {
    if(inputMatch !== null) {
