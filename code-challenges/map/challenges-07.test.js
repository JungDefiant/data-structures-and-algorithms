...
const forEachTwoToThe = (arr) => {
  arr.forEach((val, ind) => {
    // val returns a copy of a value inside the array (not the actual reference to that 'spot')
    // arr[ind] is the reference to the actual value inside that array
    arr[ind] = Math.pow(2, val);
  })
  return arr;
};
...
const mapTwoToThe = (arr) => {
  return arr.map(val => { return Math.pow(2, val) } );
};
