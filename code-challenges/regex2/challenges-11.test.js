...
// RegEx Explanation: This expression checks that the string begins with exactly 4 decimals (^\d) and has no more than 4 decimals ($).
...
const validateEmail = (email) => {
  const regexp = /^\w+(\.\w+)?\@\w+(.net|.com|.org)$/g;
  return regexp.test(email);
};

// RegEx Explanation: The expression checks if there is one or more characters and then one or more characters following a period (\w+(\.w+)?). Next it checks for '@' and then one or more characters (\@\w+). Finally, it checks for .net, .com, or .org at the end (.net | .com | .org).
...
const validatePhoneNumber = (phoneNumber) => {
  const regexp = /^(\(\d{3}\)|\d{3})(\s|\-)?\d{3}(\s|\-)?\d{4}$/g;
  return regexp.test(phoneNumber);
};

// RegEx Explanation: First, the expression checks for three numbers enclosed in parantheses or three numbers by themselves (\(\d{3}\)|\d{3}). It then checks for one or zero of a blank space or dash followed by three numbers ((\s|\-)?\d{3}). Last, it checks for one or zero of a blank space or dash followed by four numbers ((\s|\-)?\d{4}$).
